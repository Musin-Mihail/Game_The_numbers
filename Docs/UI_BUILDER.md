...
`ScrollView`, `Content` (внутри `Viewport`), `Container` (внутри `GameSpace`), `Menu` (окно — прямой ребёнок Canvas; кнопка HUD — внутри `Game/Buttons`), `Txt_Score`, `Txt_Record`, `Obj_UndoCount`, `Leaderboard`, `DisabledCounters`, `Btn_Yes` / `Btn_No`, кнопки языков `EN`/`RU`/…
